...
public class MsSQLServerDB implements IDatabase {
    ...
    private static final List<String> TYPES = new ArrayList<>(
            Arrays.asList("BOOLEAN", "INT32", "INT64", "FLOAT", "TEXT"));
    private static final List<String> VALUE_TYPES = new ArrayList<>(
            Arrays.asList("INT32", "INT64", "FLOAT"));
    ...
    private String getOneLine(long idPredix, int sensorIndex, String time, Object value){
        ...
        StringBuffer sql =
                new StringBuffer("INSERT INTO ")
                        .append(config.getDB_NAME() + "_" + sysType).append(" values (");
    ...
    public Status insertOneSensorBatch(Batch batch) throws DBConnectException {
        ...
        try {
            ...
            for (Record record : batch.getRecords()) {
                String time = format.format(record.getTimestamp());
                List<Object> values = record.getRecordDataValue();
                statement.addBatch(getOneLine(idPredix, batch.getColIndex(), time, values.get(batch.getColIndex())));
            }
            statement.executeBatch();
            statement.close();
            return new Status(true);
        ...
        try {
            ...
            for (DeviceSchema deviceSchema : deviceSchemas) {
                ...
                for(String type: TYPES){
                    ...
                    String sql =
                            "select * from "
                                    + config.getDB_NAME() + "_" + sysType
                                    + ", (select max(pk_TimeStamp) as target from "
                                    + config.getDB_NAME() + "_" + sysType +" where pk_fk_Id / "
