...
public class MySqlLog {
	...
	public void initMysql() {
		try {
			...
			if (config.IS_QUERY_TEST&&!hasTable(config.DB_SWITCH+"QueryResult")) {
				stat.executeUpdate("create table "+config.DB_SWITCH+"QueryResult(id BIGINT, experimentID INTEGER, queryNum BIGINT, point BIGINT,"
			...
			if (config.IS_QUERY_TEST&&!hasTable(config.DB_SWITCH+"QueryProcess")) {
				stat.executeUpdate("create table "+config.DB_SWITCH+"QueryProcess(id BIGINT, experimentID INTEGER, clientName varchar(50), "
			...
			if(!config.IS_QUERY_TEST&&!hasTable(config.DB_SWITCH+"InsertResult")) {
				stat.executeUpdate("create table "+config.DB_SWITCH+"InsertResult(id BIGINT, experimentID INTEGER, totalPoints BIGINT,"
			...
			if (!config.IS_QUERY_TEST&&!hasTable(config.DB_SWITCH+"InsertProcess")) {
				stat.executeUpdate("create table "+config.DB_SWITCH+"InsertProcess(id BIGINT, experimentID INTEGER, clientName varchar(50), "
			...
			/**if(config.IS_QUERY_TEST) {
				labID = getNextID(config.DB_SWITCH+"QueryResult");
			}
			else {
				labID = getNextID(config.DB_SWITCH+"InsertResult");
			}*/
	...
	public void saveInsertProcess(int index, double costTime,double totalTime,long errorPoint,String remark) {
		...
		String mysqlSql = String.format("insert into "+config.DB_SWITCH+"InsertProcess values(%d,%d,%s,%d,%f,%f,%f,%d,%s,%s,%d,%d,%d,%d,%d,%d,%s)",
				System.currentTimeMillis(),labID, "'"+Thread.currentThread().getName() +"'", index,
				costTime, totalTime,
	...
	public void saveInsertResult(long totalPoints, double time, int clientNum,
		...
		try {
			stat = mysqlConnection.createStatement();
			sql = String.format("insert into "+config.DB_SWITCH+"InsertResult values(%d,%d,%d,%f,%d,%f,%d,%f,%s,%s,%d,%d,%d,%d,%d,%d,%s)", 
	...
	public void saveQueryResult(long id, long queryNum, long point, double time, int clientNum, double rate,
		...
		try {
			stat = mysqlConnection.createStatement();
			sql = String.format("insert into "+config.DB_SWITCH+"QueryResult values(%d,%d,%d,%d,%f,%d,%f,%d,%s,%s,%d,%d,%d,%s,%d,%f,%b,%s)", id, queryNum, point,
					labID,time, clientNum, rate, errorNum,"'"+config.host+"'","'"+localName+"'", config.QUERY_CHOICE, config.QUERY_SENSOR_NUM,
					config.QUERY_DIVICE_NUM, "'"+config.QUERY_AGGREGATE_FUN+"'", config.QUERY_INTERVAL,
					config.QUERY_LOWER_LIMIT,config.IS_EMPTY_PRECISE_POINT_QUERY,"'"+remark+"'");
			stat.executeUpdate(sql);
			stat.close();
	...
	public void saveQueryProcess(int index,int point, double time,String remark) {
		String mysqlSql = String.format("insert into "+config.DB_SWITCH+"QueryProcess values(%d,%d,%s,%d,%d,%f,%f,%s,%s,%d,%d,%d,%s,%d,%f,%b,%s)",
				System.currentTimeMillis(),labID, "'"+Thread.currentThread().getName() +"'", index,
				point,time, 
				point/time,
				"'"+config.host+"'","'"+localName+"'",config.QUERY_CHOICE, config.QUERY_SENSOR_NUM,
	...
	public void insertSERVER_MODE(double cpu, double mem, double io, double net_recv, double net_send,String remark){
		...
		try {
			stat = mysqlConnection.createStatement();
			sql = String.format("insert into SERVER_MODE values(%d,%d,%s,%f,%f,%f,%f,%f,%s)",
					System.currentTimeMillis(),labID,"'"+localName+"'",cpu,mem,io,net_recv,net_send,"'"+remark+"'");
			stat.executeUpdate(sql);
			stat.close();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	private int getNextID(String name) {
		...
		try {
			Statement stat = mysqlConnection.createStatement();
			String sql = "select experimentID from"+name+" order by id desc limit 0,1;";
			ResultSet rs = stat.executeQuery(sql);  
		    if(rs.next()) { 
		    	index = rs.getInt("experimentID"); 
		    }  
		    else {
		    	index = 0;
		    }
			...
			// TODO Auto-generated catch block
			LOGGER.error("获取表{}的最后一个labID失败，because {}",name,e.getMessage());
			e.printStackTrace();
		}
		return index+1;
	}
