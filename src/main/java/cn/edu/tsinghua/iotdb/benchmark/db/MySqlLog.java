...
public class MySqlLog {
	...
	public void initMysql(long labIndex) {
		...
		try {
			...
			if (config.IS_QUERY_TEST&&!hasTable(config.DB_SWITCH+"QueryResult")) {
				stat.executeUpdate("create table "+config.DB_SWITCH+"QueryResult(id BIGINT, labID BIGINT,queryNum BIGINT, point BIGINT,"
			...
			if (config.IS_QUERY_TEST&&!hasTable(config.DB_SWITCH+"QueryProcess"+labID)) {
				stat.executeUpdate("create table "+config.DB_SWITCH+"QueryProcess"+labID+"(id BIGINT, clientName varchar(50), "
			...
			if (!config.IS_QUERY_TEST&&!hasTable(config.DB_SWITCH+"InsertProcess"+labID)) {
				stat.executeUpdate("create table "+config.DB_SWITCH+"InsertProcess"+labID+"(id BIGINT, clientName varchar(50), "
	...
	public void saveInsertProcess(int index, double costTime,double totalTime,long errorPoint,String remark) {
		String mysqlSql = String.format("insert into "+config.DB_SWITCH+"InsertProcess"+labID+" values(%d,%s,%d,%f,%f,%f,%d,%s,%s,%b,%d,%d,%d,%d,%d,%d,%s)",
				System.currentTimeMillis(), "'"+Thread.currentThread().getName() +"'", index,
				costTime, totalTime,
				(config.CACHE_NUM * config.SENSOR_NUMBER / costTime) ,
				errorPoint,"'"+config.host+"'","'"+localName+"'",config.MUL_DEV_BATCH,
	...
	public void saveQueryResult(long id, long queryNum, long point, double time, int clientNum, double rate,
		...
		try {
			stat = mysqlConnection.createStatement();
			sql = String.format("insert into "+config.DB_SWITCH+"QueryResult values(%d,%d,%d,%d,%f,%d,%f,%d,%s,%s,%d,%d,%d,%s,%d,%f,%b,%s)", id, labID,queryNum, point,
