...
package cn.edu.tsinghua.iotdb.benchmark.tdengine3;
...
public class TDengine implements IDatabase {
  ...
  private static final String TAOS_URL = "jdbc:TAOS://%s:%s/test?user=%s&password=%s";
  ...
  public void init() {
    try {
      ...
      LOGGER.error("TDengine init success.");
  ...
  public Double registerSchema(List<DeviceSchema> schemaList) throws TsdbException {
    ...
    if (config.hasWrite()) {
      ...
      try (Statement statement = connection.createStatement()) {
        //        // create database
        //        String sql = String.format(CREATE_DATABASE, testDatabaseName);
        //        LOGGER.error("Create database {}", sql);
        //        statement.execute(sql);
        // use database
        String sql = String.format(USE_DB, testDatabaseName);
        LOGGER.error("Use DB {}", sql);
        statement.execute(sql);
        ...
        LOGGER.error("Create super table: {}", superTableCreateSql);
