...
- [6. iot-benchmark的使用](#6-iot-benchmark的使用)
  - [6.1 快速开始](#61-快速开始)
  - [6.2 iot-benchmark的不同运行模式](#62-iot-benchmark的不同运行模式)
    - [6.2.1 常规测试模式之写入(单数据库)](#621-常规测试模式之写入单数据库)
  - [6.2.2 常规测试模式之查询(单数据库，不使用系统记录)](#622-常规测试模式之查询单数据库，不使用系统记录)
  - [6.2.3 常规测试模式之读写混合模式（单数据库）](#623-常规测试模式之读写混合模式（单数据库）)
  - [6.2.4 常规测试模式之读写混合模式（单数据库，查询最近写入数据）](#624-常规测试模式之读写混合模式（单数据库，查询最近写入数据）)
  - [6.2.5 常规测试模式之使用系统记录（单数据库）](#625-常规测试模式之使用系统记录（单数据库）)
  - [6.2.6 常规测试模式之测试过程持久化（单数据库）](#626-常规测试模式之测试过程持久化（单数据库）)
  - [6.2.7 生成数据模式](#627-生成数据模式)
  - [6.2.8 正确性写入模式（单数据库，外部数据集）](#628-正确性写入模式（单数据库，外部数据集）)
  - [6.2.9 正确性单点查询模式（单数据库，外部数据集）](#629-正确性单点查询模式（单数据库，外部数据集）)
  - [6.2.10 双数据库模式](#6210-双数据库模式)
  - [6.2.11 常规测试模式之写入(双数据库)](#6211-常规测试模式之写入双数据库)
  - [6.2.12 正确性单点查询模式（双数据库比较）](#6212-正确性单点查询模式（双数据库比较）)
  - [6.2.13 正确性功能查询模式（双数据库比较）](#6213-正确性功能查询模式（双数据库比较）)
