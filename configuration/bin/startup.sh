...
if [ -d "${BENCHMARK_HOME}/lib/core" ]; then
  ...
  # conf 目录也放入 classpath：SLF4J 全项目统一 2.x 生态后，默认绑定为
  # lib/core 的 slf4j-reload4j（配置走 conf/log4j.properties）；iotdb 模块
  # 自带 logback 1.3.16、dolphindb 模块自带 slf4j-log4j12，经「模块目录
  # 优先」规则确定性生效。core 目录刻意只携带一个绑定——多个绑定并存时
  # SLF4J 2.x 按 classpath 顺序选取，而 JVM 对通配符展开顺序不作保证。
