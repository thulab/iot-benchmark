...
public class ConfigDescriptor {
  ...
  private ConfigDescriptor() {
    ...
    loadPropsFrom(Arrays.asList(defaultConfUrl, userConfUrl));
    // check properties
    if (!checkConfig()) {
      System.exit(1);
    }
  ...
  // earlier configurations in the list will be overwritten by the ones after them.
  private void loadPropsFrom(List<String> urls) {
    ...
    config.setBENCHMARK_WORK_MODE(
        BenchmarkMode.getBenchmarkMode(
            properties.getProperty("BENCHMARK_WORK_MODE", config.getBENCHMARK_WORK_MODE() + "")));
    config.setTEST_MAX_TIME(
        Long.parseLong(properties.getProperty("TEST_MAX_TIME", config.getTEST_MAX_TIME() + "")));
    ...
    if (config.isIS_DOUBLE_WRITE()) {
      ...
      config.setANOTHER_USERNAME(
          properties.getProperty("ANOTHER_USERNAME", config.getANOTHER_DBConfig().getUSERNAME()));
      config.setANOTHER_PASSWORD(
          properties.getProperty("ANOTHER_PASSWORD", config.getANOTHER_DBConfig().getPASSWORD()));
      ...
      config.setIS_POINT_COMPARISON(
          Boolean.parseBoolean(
              properties.getProperty("IS_POINT_COMPARISON", config.isIS_POINT_COMPARISON() + "")));
    ...
    config.setSTRING_LENGTH(
        Integer.parseInt(properties.getProperty("STRING_LENGTH", config.getSTRING_LENGTH() + "")));
    config.setDOUBLE_LENGTH(
        Integer.parseInt(properties.getProperty("DOUBLE_LENGTH", config.getDOUBLE_LENGTH() + "")));
    ...
    if (!config.isIS_SENSOR_TS_ALIGNMENT()) {
      config.setTS_ALIGNMENT_RATIO(
          Double.parseDouble(
              properties.getProperty("TS_ALIGNMENT_RATIO", config.getTS_ALIGNMENT_RATIO() + "")));
    }
    config.setIS_CLIENT_BIND(
        Boolean.parseBoolean(
            properties.getProperty("IS_CLIENT_BIND", config.isIS_CLIENT_BIND() + "")));
    config.setCLIENT_NUMBER(
        Integer.parseInt(properties.getProperty("CLIENT_NUMBER", config.getCLIENT_NUMBER() + "")));
    ...
    config.setSQUARE_RATIO(
        Double.parseDouble(properties.getProperty("SQUARE_RATIO", config.getSQUARE_RATIO() + "")));
    config.setRANDOM_RATIO(
        Double.parseDouble(properties.getProperty("RANDOM_RATIO", config.getRANDOM_RATIO() + "")));
    ...
    config.setGROUP_NUMBER(
        Integer.parseInt(properties.getProperty("GROUP_NUMBER", config.getGROUP_NUMBER() + "")));
    ...
    config.setVECTOR(
        Boolean.parseBoolean(properties.getProperty("VECTOR", String.valueOf(config.isVECTOR()))));
    config.setIOTDB_USE_DEBUG(
        Boolean.parseBoolean(
            properties.getProperty("IOTDB_USE_DEBUG", String.valueOf(config.isIOTDB_USE_DEBUG()))));
    ...
    config.setDEVICE_NUM_PER_WRITE(
        Integer.parseInt(
            properties.getProperty("DEVICE_NUM_PER_WRITE", config.getDEVICE_NUM_PER_WRITE() + "")));
    ...
    config.setIS_COPY_MODE(
        Boolean.parseBoolean(properties.getProperty("IS_COPY_MODE", config.isIS_COPY_MODE() + "")));
    config.setIS_ADD_ANOMALY(
        Boolean.parseBoolean(
            properties.getProperty("IS_ADD_ANOMALY", config.isIS_ADD_ANOMALY() + "")));
    config.setANOMALY_RATE(
        Double.parseDouble(properties.getProperty("ANOMALY_RATE", config.getANOMALY_RATE() + "")));
    config.setANOMALY_TIMES(
        Integer.parseInt(properties.getProperty("ANOMALY_TIMES", config.getANOMALY_TIMES() + "")));
    ...
    config.setOUT_OF_ORDER_MODE(
        OutOfOrderMode.getOutOfOrderMode(
            properties.getProperty("OUT_OF_ORDER_MODE", config.getOUT_OF_ORDER_MODE().toString())));
    ...
    config.setRECORD_SPLIT(
        Boolean.parseBoolean(properties.getProperty("RECORD_SPLIT", config.isRECORD_SPLIT() + "")));
