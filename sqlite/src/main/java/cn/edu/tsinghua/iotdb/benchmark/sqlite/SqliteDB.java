...
public class SqliteDB implements IDatabase {
    ...
    private static final List<String> TYPES = new ArrayList<>(Arrays.asList("INTEGER", "REAL", "TEXT"));
    ...
    public Status insertOneSensorBatch(Batch batch) throws DBConnectException {
        ...
        try {
            ...
            for (Record record : batch.getRecords()) {
                List<Object> values = record.getRecordDataValue();
                statement.addBatch(
                        getOneLine(idPredix, batch.getColIndex(), record.getTimestamp(), values.get(batch.getColIndex())));
            }
            statement.executeBatch();
            statement.close();
            return new Status(true);
        ...
        try {
            ...
            for (DeviceSchema deviceSchema : deviceSchemas) {
                ...
                for (String type : VALUE_TYPES) {
                    String sql =
                            getHeader(
                                    aggRangeValueQuery.getAggFun(), deviceSchema.getSensors(), idPrefix, type);
                    sql = addTimeClause(sql, startTime, endTime);
