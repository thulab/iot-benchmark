...
if [ "${IS_SSH_CHANGE_PORT#*=}" = "true" ]; then
    ...
    if [ "${DB#*=}" = "IoTDB" -a "${BENCHMARK_WORK_MODE#*=}" = "insertTestWithDefaultPath" ]; then
        ...
        ssh -p $SSH_PORT $SERVER_HOST "sh $LOG_STOP_FLAG_PATH/incubator-iotdb/server/iotdb/sbin/stop-server.sh;sleep 5"
        ...
        ssh -p $SSH_PORT $SERVER_HOST "sh $LOG_STOP_FLAG_PATH/incubator-iotdb/server/iotdb/sbin/start-server.sh > /dev/null 2>&1 &"
...
#        if [ "ssh root@${ip} -f filename" ]
    ...
    if [ "${DB#*=}" = "IoTDB" -a "${BENCHMARK_WORK_MODE#*=}" = "insertTestWithDefaultPath" ]; then
        ...
        if [ $IS_TEST_BASELINE = "true" ]; then
            ...
            ssh $SERVER_HOST "sh $LOG_STOP_FLAG_PATH/incubator-iotdb/server/iotdb/sbin/stop-server.sh;sleep 5"
            #start server system information recording
            ssh $SERVER_HOST "sh $LOG_STOP_FLAG_PATH/iotdb-benchmark/ser-benchmark.sh > /dev/null 2>&1 &"
            ...
            sed -i "s/^VERSION.*$/VERSION=${COMMIT_ID}/g" $BENCHMARK_HOME/conf/config.properties
            scp $BENCHMARK_HOME/$IOTDB_CONF/iotdb-engine.properties $SERVER_HOST:$LOG_STOP_FLAG_PATH/incubator-iotdb/server/iotdb/conf
            scp $BENCHMARK_HOME/$IOTDB_CONF/iotdb-env.sh $SERVER_HOST:$LOG_STOP_FLAG_PATH/incubator-iotdb/server/iotdb/conf
            ssh $SERVER_HOST "bash $LOG_STOP_FLAG_PATH/incubator-iotdb/server/iotdb/sbin/start-server.sh > /dev/null 2>&1 &"
        fi
        echo 'wait a few seconds for lauching IoTDB...'
        sleep 40
    fi
