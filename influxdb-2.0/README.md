...
# 2. Database setup
visit http://{ip}:8086/ to set up user
1. username: admin
2. password: 12345678
...
[config.yaml for InfluxDB v2.0](config.yaml) is only a sample InfluxDB server-side configuration file, more details: https://docs.influxdata.com/influxdb/v2.7/reference/config-options/. [Syntax](https://docs.influxdata.com/influxdb/v2.0/reference/flux/)

# 3. Build benchmark

Build only the InfluxDB 2.x module and its dependencies:

```bash
mvn -pl influxdb-2.0 -am package -DskipTests
```

After packaging, the benchmark tool is generated under:

```text
influxdb-2.0/target/iot-benchmark-influxdb-2.0
influxdb-2.0/target/iot-benchmark-influxdb-2.0.zip
```

# 4. Configure benchmark

For the current `influxdb-2.0` module, the minimal items to check in [config.properties](./config.properties) are:
...
```bash
cd influxdb-2.0/target/iot-benchmark-influxdb-2.0
./benchmark.sh
```
