...
#        if [ "ssh root@${ip} -f filename" ]
    ...
    if [ "${DB#*=}" = "IoTDB" -a "${BENCHMARK_WORK_MODE#*=}" = "insertTestWithDefaultPath" ]; then
        ...
        if [ $IS_TEST_BASELINE = "true" ]; then
            ...
            #start server system information recording
            ssh $SERVER_HOST "sh $LOG_STOP_FLAG_PATH/iotdb-benchmark/ser-benchmark.sh > /dev/null 2>&1 &"
            COMMIT_ID=$(ssh $SERVER_HOST "cd $LOG_STOP_FLAG_PATH/iotdb-$BRANCH/iotdb;git tag -l | tail -n 1")"_commit_id:"$(ssh $SERVER_HOST "cd $LOG_STOP_FLAG_PATH/iotdb-$BRANCH/iotdb;git rev-parse HEAD")
            sed -i "s/^VERSION.*$/VERSION=${COMMIT_ID}/g" $BENCHMARK_HOME/conf/config.properties
            scp $BENCHMARK_HOME/$IOTDB_CONF/iotdb-engine.properties $SERVER_HOST:$LOG_STOP_FLAG_PATH/iotdb-$BRANCH/iotdb/iotdb/iotdb/conf
            scp $BENCHMARK_HOME/$IOTDB_CONF/iotdb-env.sh $SERVER_HOST:$LOG_STOP_FLAG_PATH/iotdb-$BRANCH/iotdb/iotdb/iotdb/conf
            ssh $SERVER_HOST "sh $LOG_STOP_FLAG_PATH/iotdb-$BRANCH/iotdb/iotdb/iotdb/bin/start-server.sh > /dev/null 2>&1 &"
        fi
        echo 'wait a few seconds for lauching IoTDB...'
        sleep 40
    fi
